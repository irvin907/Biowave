...
import './Ondas.css'; // Archivo de estilos para Ondas (si es necesario)
...
const Ondas = ({ datos }) => {
    ...
    useEffect(() => {
        ...
        // Dibujar las ondas basadas en los datos
        ctx.beginPath();
        ctx.moveTo(0, height / 3); // Iniciar desde el centro izquierdo

        for (let i = 0; i < datos.length; i++) {
            const dato = datos[i];
            const x = (i / datos.length) * canvasWidth;
            const y = height / 2 - dato * (height / 3); // Calcular la posición vertical
            ctx.lineTo(x, y); // Dibujar línea hasta el punto
        }

        ctx.strokeStyle = '#007BFF'; // Color de las ondas
        ctx.lineWidth = 2;
        ctx.stroke();
